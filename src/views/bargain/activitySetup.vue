...
<script>
  import { addMaterial } from '@/api/bargain'
  import { genQrcode } from '@/api/bargain'
  import UploadImg from '../../components/UploadImg'
  import UploadMusic from '../../components/UploadMusic'
  import { getToken } from '@/utils/auth'
  import { detailMaterial } from '@/api/bargain'
    ...
    methods: {
      ...
      save() {
        ...
        addMaterial(this.saveUrl)
          .then((result) => {
            result ? this.$router.push({ name: 'duckIconbargain', query: { activity_id: this.$route.query.activity_id }}) : ''
          })
      },
      // 拿到活动介绍的图片
      getImg11(data) {
    ...
    mounted() {
      ...
      detailMaterial({ activity_id })
        .then((result) => {
          ...
          this.saveUrl.group_color = result.purchase_color
          this.saveUrl.text_color = result.purchase_detail
