...
<script>
  import { addRed } from '@/api/secKill'
  import { getRedDetail } from '@/api/secKill'
  export default {
    data() {
      return {
        chooseValue: '',
    ...
    methods: {
      changeSelect(value, label) {
        this.Temporary.push({ rule_id: value.value, label: value.label, amount: '', money: ['', ''], disabled: value.disabled })
        value.disabled = true
      },
      blur1(value, index) {
        if (value < 1) {
