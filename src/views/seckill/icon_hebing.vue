    ...
    <el-form label-width="120px">
      ...
      <el-form-item label="图标上传">
        <el-col :span="24">
          ...
          <div v-for="(item, index) in showBase">
      ...
      <el-form-item label="图标个数">
        <el-input v-model="addLightBase.pic_num" @blur="icon_blur"></el-input>
        <el-col :span="24" style="margin:15px 0">
...
<script>
  ...
  import { addLight } from '@/api/secKill'
  import { getLightDetail } from '@/api/secKill'
  ...
  export default {
    ...
    methods: {
      ...
      icon_blur() {
        this.showBase = []
        this.linshiSaveData = []
        for (let icons_num = 0; icons_num < this.addLightBase.pic_num; icons_num++) {
          this.showBase[icons_num] = { dark: this.icon_arr_commit[0].dark, light: this.icon_arr_commit[0].light }
          this.linshiSaveData[icons_num] = { dark: this.icon_arr_commit[0].save_dark, light: this.icon_arr_commit[0].save_light }
        }
      },
      save() {
        ...
        addLight(this.addLightBase)
          .then((result) => {
            result ? this.$router.push({ name: 'redpacketSeckill', query: { activity_id: this.$route.query.activity_id }}) : ''
          })
      }
    },
    mounted() {
